...
import { Pool } from "pg";
import { InsertUser, InsertScan, users, scans } from "../drizzle/schema";
...
export async function getScansByUser(userId: string, limit = 50) {
  const db = await getDb();
  if (!db) return [];
  return db
    .select()
...
export async function deleteScan(id: number, userId: string) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  return db.delete(scans).where(eq(scans.id, id)).returning();
}
